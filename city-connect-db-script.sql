...
CREATE TABLE users (
    ...
    date_naissance DATE,
    role VARCHAR(50) NOT NULL DEFAULT 'citoyen' CHECK (
        role IN (
            'citoyen',
            'prestataire',
            'admin'
...
CREATE TABLE plainte (
    ...
    categorie_id INTEGER REFERENCES plainte_categorie (id) ON DELETE SET NULL
);
...
CREATE TABLE plainte_media (
    ...
    plainte_id UUID NOT NULL REFERENCES plainte (id) ON DELETE CASCADE
);
...
CREATE TABLE demande_service_document (
    ...
    file_url TEXT NOT NULL, -- Backblaze B2 URL
    file_type VARCHAR(100),
    demande_id UUID REFERENCES demande_service (id) ON DELETE CASCADE
);
...
CREATE TABLE message (
    ...
    date TIMESTAMP NOT NULL DEFAULT NOW(),
    sender VARCHAR(20) NOT NULL DEFAULT 'user' CHECK (sender IN ('user', 'bot')),
...
CREATE TABLE job_offer (
    ...
    date_creation TIMESTAMP NOT NULL DEFAULT NOW(),
    prestataire_id UUID NOT NULL REFERENCES prestataire (user_id) ON DELETE CASCADE,
...
-- =============================================================
--  FILLING THE PLAINTE_CATEGORIE TABLE WITH THE PREDEFINED VALUES
-- =============================================================

INSERT INTO plainte_categorie (type)
VALUES 
    ('Street lighting'),
    ('traffic lights'),
    ('Potholes'),
